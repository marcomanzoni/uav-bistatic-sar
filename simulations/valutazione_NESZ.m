...
% Rumore
F = 10^(F_dB/10);  % [W]
Nw = F*K*T_ref; % Densità spettrale di potenza
Pw = Nw*fs; % Potenza del rumore
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
v = 0;
v = v + 1; 
Param_str{v} = ['f_0 = ' num2str(f0*1e-9,3) ' GHz']; 
v = v + 1; 
Param_str{v} = ['P_t_x = ' num2str(P_tx_dBm,3) ' dBm']; 
v = v + 1; Param_str{v} = ['B = ' num2str(B*1e-6,3) ' MHz']; 
v = v + 1; Param_str{v} = ['G = ' num2str(10*log10(G),3) ' dB']; 
v = v + 1; Param_str{v} = ['off-nadir pointing = ' num2str(teta_pt_deg,3) ' deg'];
...
P_rx_tot =  P_rx_tot_signal + Pw; % Potenza totale 
